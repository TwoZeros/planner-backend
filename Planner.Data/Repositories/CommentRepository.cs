...
namespace Planner.Data.Repositories
{
    public class CommentRepository : BaseRepository<Planner.Models.Comment>, ICommentRepository
    {
       ...
       public List<Comment> GetClientComments(int id)
        ...
        {
            var comments = _context.Comments.Where(p => p.CreateDate.AddHours(3).Date == date.Date)
        ...
        {
            var dates= _context.Comments.Where(p=>p.ClientId== clientId).OrderBy(p => p.CreateDate).Select(p=>p.CreateDate.AddHours(3).Date).Distinct().ToList();
